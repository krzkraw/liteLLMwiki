...
### Files changed
- `internal/tui/store/store.go` — proxy action types, payloads, ChatSession
- `internal/tui/store/reducers.go` — ReduceProxy, ReduceChat session creation
- `internal/proxy/proxy.go` — SetTransport method
- `internal/server/server.go` — wire ObservingRoundTripper in Handler()

### Files created
- `internal/server/observe.go` — ObservingRoundTripper, observingBody
- `internal/server/observe_test.go` — 14 integration and unit tests

### Key decisions
- Async dispatch via goroutines — never blocks the proxy response
- Wraps body at the RoundTrip level for transparent SSE streaming support
- API sessions created with SourceOpenAI, ActiveSessionID never changed
- Observing transport is nil-safe — no-op when command bus absent
