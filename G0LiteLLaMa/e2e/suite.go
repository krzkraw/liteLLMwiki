...
func litertSkipReason(combo RuntimeBackendCombo) string {
	...
	if combo.Executable == "" {
		return "missing executable for litert; set LITERT_LM_BIN or install G0LiteLLaMa/litert-runtimes"
	}
	return ""
}
...
func llamaSkipReason(combo RuntimeBackendCombo) string {
	if combo.Executable == "" {
		return "no installed runtime variant for backend " + combo.ConfigBackend + "; set LLAMA_SERVER_BIN or install G0LiteLLaMa/llama-runtimes"
	}
	if combo.ModelPath == "" {
