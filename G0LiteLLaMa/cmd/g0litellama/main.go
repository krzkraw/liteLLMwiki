...
const (
	G0LiteLLaMaModeTUI      launchMode = "tui"
	G0LiteLLaMaModeHeadless launchMode = "headless"
)

func main() {
	addr := flag.String("addr", platform.DefaultListenAddr, "G0LiteLLaMa listen address")
	...
	modelID := flag.String("model-id", litert.DefaultModelID, "LiteRT-LM registry model ID")
	...
	log.Printf("G0LiteLLaMa listening on http://%s", *addr)
	...
	if mode == G0LiteLLaMaModeTUI {
		...
		case err := <-serverErr:
			if err != nil && !errors.Is(err, http.ErrServerClosed) {
				log.Fatalf("serve G0LiteLLaMa: %v", err)
			}
		case err := <-tuiErr:
			if err != nil && !errors.Is(err, context.Canceled) {
				log.Printf("run G0LiteLLaMa TUI: %v", err)
			}
		}
	} else {
		select {
	...
	if err := httpServer.Shutdown(shutdownCtx); err != nil {
		log.Printf("shutdown G0LiteLLaMa: %v", err)
	}
	if startDefaultRunner {
...
func G0LiteLLaMaMode(headless bool) launchMode {
	if headless {
		return G0LiteLLaMaModeHeadless
	}
	return G0LiteLLaMaModeTUI
}

func terminalLogTees(mode launchMode) (io.Writer, io.Writer) {
	if mode == G0LiteLLaMaModeHeadless {
...
func shouldStartDefaultRunner(mode launchMode) bool {
	return mode == G0LiteLLaMaModeHeadless
}
