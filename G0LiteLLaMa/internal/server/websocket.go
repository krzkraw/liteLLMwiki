...
func (s *Server) handleWebSocketAPIRequest(
	...
	switch {
	case requestPath.Path == "/g0litellama/v1/status":
	...
	case requestPath.Path == "/g0litellama/v1/models":
		if method != http.MethodGet {
			_ = writer.sendAPITextResponse(
				message.ID,
				http.StatusMethodNotAllowed,
	...
	case requestPath.Path == "/g0litellama/v1/models/download":
	...
	case requestPath.Path == "/g0litellama/v1/runners" ||
		strings.HasPrefix(requestPath.Path, "/g0litellama/v1/runners/"):
	...
	case requestPath.Path == "/g0litellama/v1/multimodal":
		if method != http.MethodPost {
			_ = writer.sendAPITextResponse(
				message.ID,
				http.StatusMethodNotAllowed,
