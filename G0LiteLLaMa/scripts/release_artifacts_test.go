...
var releaseArtifacts = []releaseArtifact{
	...
	{
		dir:      "g0litellama-windows-amd64",
		binary:   "g0litellama.exe",
		magicSet: [][]byte{{'M', 'Z'}},
	},
	{
		dir:      "g0litellama-windows-arm64",
		binary:   "g0litellama.exe",
		magicSet: [][]byte{{'M', 'Z'}},
	},
}
...
func TestShellReleaseScriptDefaultsToG0LiteLLaMaArtifacts(t *testing.T) {
	t.Parallel()
	...
	if !strings.Contains(string(content), `repo_root/G0LiteLLaMa/dist`) {
		t.Fatalf("build-release.sh does not default to G0LiteLLaMa/dist")
	}
}
...
func TestReleaseBuildCommandUsesPowerShellOnWindows(t *testing.T) {
	...
	root := filepath.Clean(`C:\repo\G0LiteLLaMa`)
