...
func TestStatusReturnsBackendEvidence(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/g0litellama/v1/status", nil)
...
func TestStatusReportsMultimodalCapabilityWhenRunnerConfigured(t *testing.T) {
	...
	if body.Capabilities.Multimodal.Endpoint != "/g0litellama/v1/multimodal" {
...
func TestModelsEndpointListsCatalog(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/g0litellama/v1/models", nil)
	rec := httptest.NewRecorder()
	...
	if len(body.Models) != 8 {
		t.Fatalf("models = %d, want 8", len(body.Models))
	}
...
func TestRunnerEndpointsCreateListAndControlRunner(t *testing.T) {
	...
	patchReq := httptest.NewRequest(
		http.MethodPatch,
		"/g0litellama/v1/runners/embedding-llamacpp",
		strings.NewReader(`{
			"backend": "gpu",
			"port": 9592,
			"verbose": false,
	...
	listReq := httptest.NewRequest(http.MethodGet, "/g0litellama/v1/runners", nil)
	...
	for _, action := range []string{"start", "restart", "stop"} {
		req := httptest.NewRequest(
			http.MethodPost,
			"/g0litellama/v1/runners/embedding-llamacpp/"+action,
			nil,
		)
		rec := httptest.NewRecorder()
		handler.ServeHTTP(rec, req)
...
func TestRunnerPatchAcceptsCommandOverrides(t *testing.T) {
	...
	commandLineReq := httptest.NewRequest(
		http.MethodPatch,
		"/g0litellama/v1/runners/main-llamacpp-command",
		strings.NewReader(`{
...
func TestRunnerCloseEndpointRemovesRunnerAndRoute(t *testing.T) {
	...
	closeReq := httptest.NewRequest(
		http.MethodPost,
		"/g0litellama/v1/runners/embedding-llamacpp/close",
		nil,
	)
	...
	missingReq := httptest.NewRequest(
		http.MethodPost,
		"/g0litellama/v1/runners/missing-runner/close",
		nil,
	)
...
func TestWebSocketAPIRequestControlsRunner(t *testing.T) {
	...
	client.WriteJSON(t, map[string]any{
		...
		"id":     "start-runner",
		"method": "POST",
		"path":   "/g0litellama/v1/runners/rerank-llamacpp/start",
	})
	...
	client.WriteJSON(t, map[string]any{
		...
		"id":         "patch-runner",
		"method":     "PATCH",
		"path":       "/g0litellama/v1/runners/rerank-llamacpp",
		"headers":    map[string]string{"content-type": "application/json"},
...
func TestUpstreamFailureReportsLastError(t *testing.T) {
	...
	statusReq := httptest.NewRequest(http.MethodGet, "/g0litellama/v1/status", nil)
...
func TestWebSocketAPIRequestListsModelCatalog(t *testing.T) {
	...
	if len(response.Models) != 8 {
		t.Fatalf("models = %d, want 8", len(response.Models))
	}
}
...
func TestWebSocketAPIRequestRunsMultimodal(t *testing.T) {
	...
	client.WriteJSON(t, map[string]any{
		...
		"id":         "request-1",
		"method":     "POST",
		"path":       "/g0litellama/v1/multimodal",
		"headers":    map[string]string{"content-type": "application/json"},
