...
- `package.json` - Node scripts and dependency versions.
...
`npm run smoke:model` requires an external model file under `models/`. Real
...
- Never commit `.litertlm` files, `.litertlm.parts/`, partial downloads, or
  model caches.
- Never commit `public/vendor/`; it is regenerated by `npm install`.
