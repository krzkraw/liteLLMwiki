...
func TestManagerStartModeWithConfigAppliesRuntimeConfig(t *testing.T) {
	t.Parallel()
	...
	if err := manager.StartModeWithConfig(ctx, RuntimeModeRelease, ConfigPatch{
		...
		t.Fatalf("start with runtime config: %v", err)
	}
	args := readRuntimeHelperArgs(t, argsFile, &helperOutput, 1)
	stopCtx, cancel := context.WithTimeout(ctx, 2*time.Second)
	defer cancel()
	...
	if !strings.Contains(args, "serve --host 127.0.0.1 --port 9481") {
		t.Fatalf("start args = %q, want selected host and port", args)
	}
	if strings.Contains(args, "--verbose") {
	...
	if status.Upstream != "http://127.0.0.1:9481" {
		t.Fatalf("upstream = %q, want selected port", status.Upstream)
	}
}

func TestManagerConfigSnapshotReflectsRuntimeConfig(t *testing.T) {
	t.Parallel()
	...
	if snapshot.HuggingFaceToken != "hf_secret" {
		t.Fatalf("hugging face token = %q, want configured token", snapshot.HuggingFaceToken)
	}
	huggingFaceToken = ""
	if err := manager.ApplyConfigPatch(ConfigPatch{
		HuggingFaceToken: &huggingFaceToken,
