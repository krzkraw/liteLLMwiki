...
func TestG0LiteLLaMaModeDefaultsToTUI(t *testing.T) {
	t.Parallel()

	if got := G0LiteLLaMaMode(false); got != G0LiteLLaMaModeTUI {
		t.Fatalf("default mode = %q, want TUI", got)
	}
	if got := G0LiteLLaMaMode(true); got != G0LiteLLaMaModeHeadless {
...
func TestTerminalLogTeesAreDisabledInTUIMode(t *testing.T) {
	...
	stdout, stderr := terminalLogTees(G0LiteLLaMaModeTUI)
...
func TestTerminalLogTeesRemainEnabledInHeadlessMode(t *testing.T) {
	...
	stdout, stderr := terminalLogTees(G0LiteLLaMaModeHeadless)
...
func TestDefaultRunnerIsLazyInTUIMode(t *testing.T) {
	...
	if shouldStartDefaultRunner(G0LiteLLaMaModeTUI) {
		t.Fatalf("TUI mode should not start a default runner before the launch wizard")
	}
	if !shouldStartDefaultRunner(G0LiteLLaMaModeHeadless) {
