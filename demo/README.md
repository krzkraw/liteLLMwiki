...
Install dependencies from `demo/`:
...
The Vite dev and preview servers serve `/models/*` from `demo/models/`, so the
large `.litertlm` files do not need to be copied into `public/`.
...
source lives in `../native/sidecar`, but the built native runner used by this
web UI is placed under `native/sidecar/`.

Build the webUI-local native runner artifacts from `demo/`:
...
for backend evidence. The base
`gemma4-e2b` model means the default CPU/base path is available; advertised
`gemma4-e2b,gpu` and `gemma4-e2b,npu` IDs enable those UI selections. CUDA is
shown as probe-only and is not formatted into LiteRT-LM model strings.
