...
describe("model file script paths", () => {
  it("defaults web model tools to the repository-local models directory", async () => {
    ...
    expect(resolveWebModelPath("/repo/demo")).toBe(
      resolve("/repo/demo", "models/gemma-4-E2B-it-web.litertlm"),
    );
  });
  ...
  it("honors an explicit model path argument", async () => {
    ...
    expect(resolveWebModelPath("/repo/demo", "../models/custom.litertlm")).toBe(
      resolve("/repo/demo", "../models/custom.litertlm"),
    );
  });
});
